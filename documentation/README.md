# 📚 Documentation Index

Welcome to the Price Tracker documentation! This folder contains detailed guides and references.

## 🚀 Getting Started

**New to the project?** Start here:
- **[../README.md](../README.md)** - Main project overview
- **[../QUICK_START.md](../QUICK_START.md)** - Quick setup guide

## 📖 User Guides

- **[QUICK_USER_GUIDE.md](QUICK_USER_GUIDE.md)** - How to use the application
- **[DEMO_SUMMARY.md](DEMO_SUMMARY.md)** - Visual demo walkthrough

## 🛠️ Developer Guides

### Scraping & Integration
- **[SCRAPING_SERVICES_GUIDE.md](SCRAPING_SERVICES_GUIDE.md)** - Complete guide to Bright Data integration
- **[QUICK_SETUP_SCRAPING.md](QUICK_SETUP_SCRAPING.md)** - Quick reference for scraping setup
- **[CUSTOM_SCRAPERS_GUIDE.md](CUSTOM_SCRAPERS_GUIDE.md)** - How to add vendor-specific scrapers
- **[SCRAPERS_QUICK_REFERENCE.md](SCRAPERS_QUICK_REFERENCE.md)** - Scraper development reference

### Architecture & Implementation
- **[PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md)** - System architecture
- **[SCRAPER_IMPLEMENTATION_SUMMARY.md](SCRAPER_IMPLEMENTATION_SUMMARY.md)** - Scraper implementation details
- **[SCRAPER_WORKFLOW.md](SCRAPER_WORKFLOW.md)** - Visual workflow diagrams
- **[FRONTEND_SCRAPER_INTEGRATION.md](FRONTEND_SCRAPER_INTEGRATION.md)** - Frontend integration guide

## 📝 Recent Updates

- **[../FEATURES.md](../FEATURES.md)** - Latest features and improvements (Dec 2025)
- **[WHAT_WAS_ADDED.md](WHAT_WAS_ADDED.md)** - Historical change log

---

**Need help?** Start with the main [README](../README.md) or [QUICK_START](../QUICK_START.md)!

